...
def main():
    ...
    manifest_path = curated_root / "processed-manifest.json"
    ...
    if manifest.get("chapter_id") != CHAPTER_ID:
        failures.append("curated manifest chapter_id mismatch")
    ...
    for key in ("scene_reviewed", "source_order_reviewed", "terminology_consistent", "character_voice_reviewed"):
        if review.get(key) is not True:
            failures.append(f"translation review flag {key}=false")
    ...
    if len(active) != EXPECTED_ACTIVE:
        failures.append(f"manifest active story object count={len(active)}, expected {EXPECTED_ACTIVE}")
    if missing_ids:
    ...
    summary = {
        ...
        "terminology_consistent": bool(review.get("terminology_consistent")),
        "character_voice_reviewed": bool(review.get("character_voice_reviewed")),
        "system_ui_reviewed": bool(review.get("system_ui_reviewed")),
        "failures": failures,
