...
def reference_old(detector, candidates, prototypes):
    ...
    for candidate in candidates:
        ...
        _score, _cid, _num_classes, geometry, coeffs = detector._candidate_fields(candidate)
...
def run(*, correctness_only: bool = False):
    ...
    check(
        ...
        "optimized masks/geometry differ from reference",
    )
    ...
    for kept_index in kept:
        ...
        bounds = [
            detector._prototype_crop_bounds(candidates[index][7], prototypes)
        ...
        peak_batch_elements = max(
            peak_batch_elements,
            min(8, len(members)) * roi,
        )
    check(roi_elements < full_elements, "ROI plan did not reduce prototype work")
    check(
        peak_batch_elements < 2 * prototypes.shape[1] * prototypes.shape[2],
        "ROI batch temporary exceeds old logits+probability footprint",
    )

    if correctness_only:
        print(
            "phase5 correctness PASS "
            f"work={full_elements}->{roi_elements} "
            f"peak_elements<={peak_batch_elements}"
        )
    ...
    check(
        new_ms < old_ms,
        f"dense postprocess did not improve: old={old_ms:.2f}ms new={new_ms:.2f}ms",
    )
    print(
        f"phase5 dense postprocess reference={old_ms:.2f}ms "
        f"optimized={new_ms:.2f}ms work={full_elements}->{roi_elements} "
        f"peak_elements<={peak_batch_elements}"
    )
    print("backend detector postprocess sanity: phase 5 PASS")
...
if __name__ == "__main__":
    ...
    run(correctness_only=args.correctness_only)
