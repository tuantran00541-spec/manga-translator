#!/usr/bin/env python3
"""Exact post-SwiGLU hotspot profile for the current best Qwen3.8 staged prefill path.

This profile keeps all already-proven exact native changes enabled:
  * multi-vector quantized matvec;
  * native F32 matvec;
  * native full-attention causal core;
  * native GDN causal-conv + SiLU;
  * native GDN output RMSNorm + SiLU gate;
  * native FFN SwiGLU.

The profiler measures every matvec-many call by tensor role, activation preparation,
FFN sub-stages, the four native helper families, and reports the remaining
unprofiled Python/orchestration time. Hidden vectors, persistent decoder state,
known SHA anchors, direct-I/O, and one-stream K3 traffic remain hard gates.
"""
...
class Profile:
    ...
    def record_ffn(self, prefix: str, *, total: float, prepare: float, gate: float,
                   up: float, swiglu: float, down: float) -> None:
        ...
        values = {
            ...
            "up_matvec": float(up),
            "swiglu_native": float(swiglu),
            "down_matvec": float(down),
        }
        self.ffn_layers[layer] = {"layer": layer, "kind": kind, **values}
    ...
    def report(self) -> dict[str, Any]:
        ranking = [
            {"stage": stage, "seconds": seconds, "calls": self.ffn_stage_calls[stage]}
            for stage, seconds in sorted(self.ffn_stage_seconds.items(), key=lambda kv: kv[1], reverse=True)
        ]
        return {
            "ffn_stage_seconds": dict(self.ffn_stage_seconds),
            "ffn_stage_calls": dict(self.ffn_stage_calls),
            "ffn_stage_ranking": ranking,
...
def sanity() -> None:
    if layer_kind(0) != "gdn" or layer_kind(3) != "attention" or layer_kind(63) != "attention":
    ...
    print("QWEN38_POST_SWIGLU_PROFILE_SANITY PASS")
...
def run(args) -> dict[str, Any]:
    ...
    try:
        ...
        def profiled_ffn(runtime, view, metas, prefix, xs):
            ...
            t0 = time.monotonic()
            gate = runtime.matvec_many(view("ffn_gate.weight"), metas[f"{prefix}.ffn_gate.weight"], xs, prepared=prepared)
            gate_seconds = time.monotonic() - t0

            t0 = time.monotonic()
            up = runtime.matvec_many(view("ffn_up.weight"), metas[f"{prefix}.ffn_up.weight"], xs, prepared=prepared)
            up_seconds = time.monotonic() - t0

            t0 = time.monotonic()
            sw = swiglu_core.compute(gate, up)
            swiglu_seconds = time.monotonic() - t0
            native_stats["native_swiglu_seconds"] += swiglu_seconds

            t0 = time.monotonic()
            out = runtime.matvec_many(view("ffn_down.weight"), metas[f"{prefix}.ffn_down.weight"], sw)
            down_seconds = time.monotonic() - t0

            profiler.record_ffn(prefix, total=time.monotonic() - t_total, prepare=prepare_seconds,
                                gate=gate_seconds, up=up_seconds, swiglu=swiglu_seconds, down=down_seconds)
            return out
        ...
        def native_attention(engine_, hidden, il, pos0, view, metas, vec):
            ...
            out = attn_probe._full_attention_layer_native(engine_, hidden, il, pos0, view, metas, vec, attention_core, local)
        ...
        def native_recurrent(engine_, hidden, il, view, metas, vec):
            ...
            out = gate_probe._recurrent_layer_native_conv_gate(conv_core, output_gate_core, local,
        ...
        if hidden_sha != KNOWN_HIDDEN_SHA256:
            raise RuntimeError(f"post-SwiGLU profile hidden anchor changed: {hidden_sha}")
        if state_sha != KNOWN_STATE_SHA256:
            raise RuntimeError(f"post-SwiGLU profile state anchor changed: {state_sha}")
        if k3_bytes != K3_STREAM_BYTES:
            raise RuntimeError(f"post-SwiGLU profile expected one K3 stream, got {k3_bytes}")
        reader = engine.reader.report()
        if not bool(reader.get("direct_io")):
            raise RuntimeError("post-SwiGLU profile requires direct I/O")
        ...
        if swiglu_report["calls"] != 64 or swiglu_report["rows"] != 64 * len(PROMPT_IDS):
            raise RuntimeError(f"unexpected native SwiGLU coverage: {swiglu_report}")

        profile = profiler.report()
        total_matvec_seconds = sum(profiler.role_seconds.values())
        total_prepare_seconds = sum(profiler.prepare_seconds.values())
        total_native_seconds = sum(native_stats.values())
        unprofiled_seconds = prefill_seconds - total_matvec_seconds - total_prepare_seconds - total_native_seconds
        payload = {
            "schema": "qwen38-post-swiglu-profile-v1",
            "status": "PASS",
            "claim": "exact post-SwiGLU hotspot profile; hidden/state anchors verified bitwise",
            ...
            "total_matvec_seconds": total_matvec_seconds,
            "total_prepare_seconds": total_prepare_seconds,
            "total_native_helper_seconds": total_native_seconds,
        ...
        print("QWEN38_POST_SWIGLU_PROFILE_BITWISE_PASS")
        return payload
    finally:
        engine.close()
...
def parser() -> argparse.ArgumentParser:
    ...
    for name in ("model", "quant-lib", "many-lib", "state-lib", "f32-lib", "attn-lib", "conv-lib", "gate-lib", "swiglu-lib", "inventory", "work-dir", "output"):
