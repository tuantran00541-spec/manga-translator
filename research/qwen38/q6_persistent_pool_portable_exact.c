/* Exact portable persistent row-parallel Q6_K matvec-many experiment.
 *
 * This is the cross-platform synchronization twin of q6_persistent_pool_exact.c.
 * It preserves the same static disjoint output-row partition and calls the same
 * proven exact Q6 many-vector kernel for each slice.  Only thread/mutex/condvar
 * plumbing changes: POSIX pthreads on Unix and native Win32 primitives on
 * Windows.  Q8_0 remains the proven noalloc bridge.
 */
#ifndef _WIN32
...
/* Keep the proven noalloc implementation internal, then re-export a stable
 * cross-platform ABI explicitly so Windows DLL consumers do not depend on
 * linker-specific export-all behavior. */
#define qwen_matvec_many_q8_0_q8_0_bridge qwen_portable_internal_q8_bridge
#define qwen_matvec_many_q6_k_q8_k_bridge qwen_portable_internal_q6_bridge
#include "gguf_quant_matvec_many_noalloc_bridge.c"
#undef qwen_matvec_many_q8_0_q8_0_bridge
#undef qwen_matvec_many_q6_k_q8_k_bridge

QWEN_EXPORT int qwen_matvec_many_q8_0_q8_0_bridge(
        ...
        float *out) {
    return qwen_portable_internal_q8_bridge(
        weights, weights_bytes, rows, n,
        activations, activation_bytes_each, n_vec, out);
}

QWEN_EXPORT int qwen_matvec_many_q6_k_q8_k_bridge(
        const uint8_t *weights, size_t weights_bytes, size_t rows, size_t n,
        const uint8_t *activations, size_t activation_bytes_each, size_t n_vec,
        float *out) {
    return qwen_portable_internal_q6_bridge(
        weights, weights_bytes, rows, n,
        activations, activation_bytes_each, n_vec, out);
}
...
static int qwen_q6_pool_compute_worker(qwen_q6_worker *w) {
    ...
    if (needed > w->scratch_floats) return -20;
    if (p->n % QWEN_QK_K != 0) return -21;
...
static QWEN_THREAD_RET qwen_q6_pool_worker_main(void *opaque) {
    ...
    for (;;) {
        while (!p->stop && p->generation == seen_generation) {
            if (qwen_cond_wait(&p->work_cond, &p->mutex) != 0) {
                p->stop = 1;
...
QWEN_EXPORT void *qwen_q6_pool_create(int n_threads, size_t max_rows, size_t max_vec) {
    ...
    const size_t max_local_rows =
        (max_rows + (size_t)n_threads - 1) / (size_t)n_threads;
    ...
    for (int i = 0; i < n_threads; ++i) {
        ...
        if (!p->workers[i].scratch) {
            ...
            qwen_cond_broadcast(&p->work_cond);
            qwen_mutex_unlock(&p->mutex);
            for (int j = 1; j < i; ++j) (void)qwen_thread_join(p->workers[j].thread);
...
QWEN_EXPORT void qwen_q6_pool_destroy(void *opaque) {
...
QWEN_EXPORT int qwen_q6_pool_matvec_many(
        void *opaque,
        const uint8_t *weights, size_t weights_bytes, size_t rows, size_t n,
        const uint8_t *activations, size_t activation_bytes_each, size_t n_vec,
        float *out) {
    qwen_q6_pool *p = (qwen_q6_pool *)opaque;
    if (!p || !weights || !activations || !out || rows == 0 || n_vec == 0 || n == 0) {
        return -1;
    }
    ...
    while (p->done_workers != p->n_threads - 1) {
        if (qwen_cond_wait(&p->done_cond, &p->mutex) != 0) {
            qwen_mutex_unlock(&p->mutex);
            return -6;
        }
    }
...
QWEN_EXPORT uint64_t qwen_q6_pool_calls(void *opaque) {
    const qwen_q6_pool *p = (const qwen_q6_pool *)opaque;
    return p ? p->calls : 0;
}

QWEN_EXPORT int qwen_q6_pool_threads(void *opaque) {
    const qwen_q6_pool *p = (const qwen_q6_pool *)opaque;
...
static void qwen_pool_pack_q6(uint8_t out[QWEN_BLOCK_Q6_K]) {
    ...
    for (int i = 0; i < 16; ++i) {
        scales[i] = (int8_t)((qwen_pool_u32() >> 24) % 31 - 15);
    }
    for (int i = 0; i < 256; ++i) {
        quants[i] = (int8_t)((qwen_pool_u32() >> 24) % 64 - 32);
    }
...
int main(void) {
    ...
    for (int v = 0; v < NV; ++v) {
        ...
        if (qwen_quantize_q8_k_scalar(
                x, N, acts + (size_t)v * ar, ar) != 0) return 31;
    }
    ...
    for (int it = 0; it < 3; ++it) {
        ...
        const double t0 = qwen_wall_seconds();
        const int rc = qwen_q6_pool_matvec_many(
            pool, weights, (size_t)ROWS * wr, ROWS, N,
            acts, ar, NV, cand);
        const double sec = qwen_wall_seconds() - t0;
        if (rc != 0) {
            fprintf(stderr, "portable persistent Q6 pool rc=%d threads=%d\n", rc, nth);
            qwen_q6_pool_destroy(pool); return 40 + it;
        }
        if (memcmp(ref, cand, (size_t)NV * ROWS * sizeof(float)) != 0) {
            fprintf(stderr, "portable persistent Q6 pool bitwise mismatch threads=%d\n", nth);
        ...
        printf("Q6 portable persistent-pool synthetic threads=%d seconds=%.6f\n", nth, sec);
        qwen_q6_pool_destroy(pool);
    }

    puts("QWEN38_Q6_PORTABLE_PERSISTENT_POOL_BITWISE_PASS");
