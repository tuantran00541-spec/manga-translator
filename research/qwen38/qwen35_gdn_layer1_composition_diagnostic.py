...
This is intentionally diagnostic, not a relaxed correctness gate.  It runs the
same recurrent layer 1 with several inputs/rounding policies so we can separate
(1) layer-1 semantic/layout errors from (2) amplification of the tiny layer-0
candidate error and (3) missing F32 materialization between ggml graph ops.
"""
...
def run_layer1(runtime: gdn.QuantRuntime, view, metas: dict[str, dict[str, Any]], vec,
               hidden: Sequence[float], *, strict: bool) -> dict[str, list[float]]:
    p = "blk.1"
    cp: dict[str, list[float]] = {}

    attn_norm = _maybe(gdn.rms_norm(hidden, vec("attn_norm.weight")), strict)
    cp["attn_norm-1"] = attn_norm
    ...
    cp["linear_attn_qkv_mixed-1"] = qkv
    cp["z-1"] = z
    cp["beta-1"] = beta_raw
    cp["alpha-1"] = alpha

    beta = _maybe([gdn.sigmoid(x) for x in beta_raw], strict)
    cp["beta_sigmoid-1"] = beta
    ...
    cp["q_conv_predelta-1"] = gdn.flatten([h for _ in range(repeats) for h in qn])
    cp["k_conv_predelta-1"] = gdn.flatten([h for _ in range(repeats) for h in kn])
    cp["v_conv_predelta-1"] = v
    ...
    cp["ffn_out-1"] = ffn_out
    cp["post_ffn-1"] = _maybe([residual[i] + ffn_out[i] for i in range(gdn.HIDDEN)], strict)
    return cp
...
def execute(model: Path, native_lib: Path, oracle_json: Path, work_dir: Path, output: Path) -> dict[str, Any]:
    ...
    with K3Trunk(trunk, manifest_path, budget_bytes=budget, want_ring=1, max_pinned=0, prefer_direct_io=True) as reader:
        b0 = reader.bind(0); m0 = _layer_meta(manifest, 0)
        def v0(s: str): return reader.tensor_view(b0, f"blk.0.{s}")
        def x0(s: str): return gdn.f32_vector(v0(s))
        candidate0 = full64._run_recurrent_layer(runtime, v0, m0, x0, embed, 0)
        b0.release()

        b1 = reader.bind(1); m1 = _layer_meta(manifest, 1)
        ...
        for name, (inp, strict) in inputs.items():
            cp = run_layer1(runtime, v1, m1, x1, inp, strict=strict)
...
def sanity() -> None:
    ...
    if len(MODES) != 5 or len(set(MODES)) != 5:
        raise SystemExit("mode contract failed")
    print(json.dumps({"schema":"qwen38-gdn-layer1-composition-sanity-v1","status":"PASS","modes":MODES}, indent=2))
...
def main() -> None:
    ...
    run.add_argument("--model",type=Path,required=True); run.add_argument("--native-lib",type=Path,required=True)
    run.add_argument("--oracle",type=Path,required=True); run.add_argument("--work-dir",type=Path,required=True)
