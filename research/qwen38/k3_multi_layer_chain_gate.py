#!/usr/bin/env python3
"""Exact multi-layer Qwen3.8 K3 streaming chain gate.

The reference loads official Transformers decoder layers one at a time. The
candidate packs the same official BF16 layers into one K3 trunk, keeps only two
ring slots, prefetches the next layer while computing the current layer, and
compares the final hidden state exactly.

This still stops below token generation: no token embedding, final norm, LM
head, tokenizer, vision encoder, or prompt is used.
"""
...
def main() -> None:
    ...
    parser.add_argument("--end-layer", type=int, default=7)
    parser.add_argument("--seq-len", type=int, default=4)
    parser.add_argument("--seed", type=int, default=20260831)
    parser.add_argument("--delete-source-shards-after-reference", action="store_true")
    ...
    layers = tuple(range(args.end_layer + 1))
    ...
    unsupported = sorted(set(actual_types) - {"linear_attention", "full_attention"})
    if unsupported:
    ...
    out_bin = trunk_dir / f"layers0-{args.end_layer}.trunk.bin"
    out_idx = trunk_dir / f"layers0-{args.end_layer}.trunk.json"
    ...
    disk_before_source_cleanup = shutil.disk_usage(root).free
    deleted_source_bytes = 0
    deleted_source_files: list[str] = []
    if args.delete_source_shards_after_reference:
        for shard in shards:
            path = model_dir / shard
            if path.is_file():
                deleted_source_bytes += path.stat().st_size
                path.unlink()
    ...
    with K3Trunk(
        ...
        report = trunk.report()
        if trunk.plan.ring_slots != 2 or not report["async_prefetch_enabled"]:
            raise RuntimeError(
                "multi-layer chain requires two safe ring slots with async prefetch enabled"
            )
    ...
    prefetch_ok = all(event["prefetch_accepted"] for event in prefetch_events[:-1])
    ...
    passed = (
        ...
        and prefetch_ok
        and runtime_report["bytes_read"] == expected_bytes
    )

    result = {
        "schema": "qwen38-k3-multi-layer-chain-gate-v1",
        ...
        "layer_read_bytes": {
            str(layer): int(meta_by_layer[layer]["read_bytes"])
            for layer in layers
        },
        ...
        "source_cleanup": {
            "enabled": bool(args.delete_source_shards_after_reference),
            "deleted_files": deleted_source_files,
            "deleted_bytes": deleted_source_bytes,
            "disk_free_before_bytes": disk_before_source_cleanup,
            "disk_free_after_bytes": disk_after_source_cleanup,
        },
        "total_seconds": time.monotonic() - started,
        "reference": f"official Transformers decoder layers 0-{args.end_layer} loaded sequentially",
        "candidate": (
            f"two-slot K3Trunk streamed layers 0-{args.end_layer} "
            "with next-layer async prefetch"
        ),
