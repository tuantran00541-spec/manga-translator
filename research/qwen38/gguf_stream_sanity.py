...
def build_fixture(path: Path) -> dict[str, int]:
    alignment = 64
    # Includes an unselected tokenizer-like string array to prove skip-without-materialize.
    ...
    q6_bytes = tensor_nbytes((256, 2), GGML_TYPE_Q6_K)[0]   # 420
    q8_bytes = tensor_nbytes((32, 3), GGML_TYPE_Q8_0)[0]    # 102
    f32_bytes = tensor_nbytes((4,), GGML_TYPE_F32)[0]        # 16

    rel_q6 = 0
    rel_q8 = align_up(rel_q6 + q6_bytes, alignment)
    ...
    # Distinct markers let us validate that computed spans address the intended bytes.
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="qwen38-gguf-") as tmp:
        ...
        checks.append("tensor block-size accounting")
    ...
    result = {
        "schema": "qwen38-gguf-stream-sanity-v1",
        "status": "PASS",
        "checks": checks,
