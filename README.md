...
The native renderer includes a catalog of 70 OFL-licensed comic fonts in `app/static/fonts/`, grouped into dialogue, emphasis, thought, narration, skill, SFX, horror, and romance folders. Use `GET /api/fonts` for stable `font_id` values. Set an object's `font_selection_mode` to `user`, `ai`, or `auto`; automatic mode compares the original lettering crop with the installed catalog and falls back safely when confidence is low. Legacy objects with an unmarked `default` style keep the historical fallback until auto is requested. Generate a visual contact sheet with `python scripts/font_specimen.py` and run `python scripts/font_library_sanity.py` before packaging.
