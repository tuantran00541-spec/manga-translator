...
def _prepare_runtime(chapter_id: str, import_root: Path, clean_root: Path, repair_root: Path) -> set[str]:
    raw_src = import_root / "raw"
    clean_src = clean_root / "processed"
    repair_json = repair_root / "clean-repair.json"
    overlay_src = repair_root / "overlay" / "processed"
    if not raw_src.is_dir() or not (clean_src / "manifest.json").is_file() or not repair_json.is_file():
    ...
    if overlay_src.is_dir():
        for src in overlay_src.iterdir():
            if src.is_file():
                shutil.copy2(src, processed_dst / src.name)
    ...
    if len(manifest.get("pages", [])) != EXPECTED_SLICES:
        raise RuntimeError(f"expected {EXPECTED_SLICES} reviewed slices, got {len(manifest.get('pages', []))}")
    ...
    if len(drop_ids) != EXPECTED_TOMBSTONES:
        raise RuntimeError(f"expected {EXPECTED_TOMBSTONES} durable non-story drops, got {len(drop_ids)}")
...
def _reconcile_text_objects(chapter_id: str, drop_ids: set[str]):
    ...
    save_manifest_raw(chapter_id, manifest)
    return created, _assert_tombstones(load_manifest_raw(chapter_id), drop_ids)
...
def _scene_rows(rows):
    return [r for r in rows if r["ocr_eligible"] and r["center_owned"] and not r["human_review_drop"]]
...
def main():
    ...
    drop_ids = _prepare_runtime(args.chapter_id, args.import_root, args.clean_root, args.repair_root)
