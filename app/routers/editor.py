...
def set_script_review(req: ScriptReviewRequest) -> dict:
    ...
    try:
        with get_page_lock(req.chapter_id, req.page_index), get_manifest_lock(req.chapter_id):
            ...
            if req.page_index < 0 or req.page_index >= len(pages):
                raise HTTPException(400, f"Invalid page_index: {req.page_index}")
            changed = apply_script_review(
                pages[req.page_index],
                req.object_id,
                reviewed=req.reviewed,
            )
            if manifest.get("script_review_required") is not True:
...
def set_final_review(req: FinalReviewPageRequest) -> dict:
    validate_chapter_id(req.chapter_id)
    try:
        with get_page_lock(req.chapter_id, req.page_index), get_manifest_lock(req.chapter_id):
            manifest = load_manifest_raw(req.chapter_id)
            ...
            if req.page_index < 0 or req.page_index >= len(pages):
                raise HTTPException(400, f"Invalid page_index: {req.page_index}")
            changed = apply_final_review(
                pages[req.page_index],
                approved=req.approved,
            )
            if manifest.get("final_review_required") is not True:
