...
def _component_radius(component: np.ndarray, min_radius: int, max_radius: int) -> int:
    """Estimate a conservative expansion radius from observed stroke thickness.

    The seed segmentation remains authoritative. Distance transform is used only
    to size dilation; it never creates a new component or removes seed pixels.
    """
    ...
    # A high percentile is more stable than max() around glyph junctions while
    # still distinguishing thin dialogue fonts from thick outlined/SFX glyphs.
...
def _guard_radius_for_artwork(
    ...
    """Reduce expansion when the immediate outside ring contains busy artwork."""
    ...
    # Busy rings are exactly where over-expansion can eat hair/line-art. Bias
    # toward under-erasing here; residual text is reviewable, artwork loss is not.
...
def refine_stroke_mask(
    ...
    *,
    ...
    """Conservatively expand a verified text mask using stroke-width evidence.

    Safety invariants:
      * seed pixels are never removed;
      * no disconnected candidate pixels are invented;
      * expansion is bounded by ``max_radius``;
      * if ``safe_envelope`` is supplied, only *new* pixels are clipped to it.

    This is an experimental adaptation of the stroke-mask principle from text
    erasing research, not an implementation of the paper's neural network.
    """
    ...
    if source_pixels == 0:
        stats = StrokeRefinementStats(0, 0, 0, 0, 0.0)
        return source, stats
    ...
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(source, connectivity=8)
    refined = source.copy()
    radii: list[int] = []
    ...
    for label in range(1, num_labels):
        ...
        target = refined[y1:y2, x1:x2]
        target[expanded] = 255

    # The safe envelope only restricts new growth; the detector's verified seed
    # is always preserved even if an externally supplied envelope is imperfect.
    refined[source > 0] = 255
    refined_pixels = int(np.count_nonzero(refined))
    stats_out = StrokeRefinementStats(
