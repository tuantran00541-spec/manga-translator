...
class CombinedTextDetector:
    ...
    def _refine_and_split_tall_boxes(boxes: list[BubbleBox], img: np.ndarray) -> list[BubbleBox]:
        """ Detector masks are evidence. A missing mask must stay missing all the way to the artwork-safe mask builder; turning ``None`` into a filled rectangle here would bypass the downstream safety policy and erase artwork. """
