...
class Inpainter:
    ...
    def _lama_fill(self, image: np.ndarray, crop: np.ndarray, local_mask: np.ndarray, crop_box: tuple, feather: bool = False) -> np.ndarray:
        ...
        if feather:
            ...
            # reaching this method. Do not let a blurred alpha create writes
            # outside that approved authority envelope: strict pixel safety
            # is more important than a soft transition at its boundary.
            core = local_mask > 127
            mask_3d = core[:, :, None]
