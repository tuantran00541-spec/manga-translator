...
def detected_memory_status() -> tuple[int | None, int | None]:
    """ Prefer a finite cgroup budget when present, then clamp it by the host's available memory. This keeps the arena policy safe inside the 4 GiB test container while still working on ordinary desktop installs. """
