    ...
    for page_index, page in enumerate(manifest.get("pages") or []):
        ...
        candidate_regions.extend(
            extract_candidate_regions(
                page,
                page_index,
                manual_mask=manual_masks.get(page_index),
                margin=margin,
                merge_gap=merge_gap,
                deep_area_ratio=deep_area_ratio,
            )
        )

    deep_region_ids = tuple(region.region_id for region in candidate_regions if region.requires_deep_qc)
    return ChapterQCPlan(
        global_regions=tuple(global_regions),
        candidate_regions=tuple(candidate_regions),
        deep_region_ids=deep_region_ids,
        skipped_pages=tuple(skipped_pages),
    )


def chunk_regions(regions: tuple[QCRegion, ...] | list[QCRegion], *, batch_size: int, work_prefix: str, mode: str = "region-clean") -> list[QCWorkItem]:
    if batch_size < 1:
    ...
    for start in range(0, len(ordered), int(batch_size)):
        ...
        items.append(
            QCWorkItem(
                work_id=f"{work_prefix}-{len(items) + 1:04d}",
                region_ids=tuple(region.region_id for region in batch),
