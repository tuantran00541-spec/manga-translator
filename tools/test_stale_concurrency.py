...
def test_1_delete_one_by_one_regression() -> None:
    ...
    initial_boxes = [
        ...
        {"x1": 90, "y1": 90, "x2": 120, "y2": 120, "confidence": 1.0, "mask": None},  # C (idx 2)
    ]
    pipeline, p0 = create_test_chapter(cid, initial_boxes=initial_boxes)
    try:
        # Step 1: Start a stale process_pages in a background thread that captures A, B, C
        ...
        # Wait until process_pages has captured snapshot and finished expensive work
        process_started.wait(timeout=5)

        # Step 2: Delete box A (index 0) while process_pages is running
        ...
        # Step 3: Let stale process finish and attempt commit
        mutation_done.set()
        t1.join(timeout=5)

        # Verify: stale result must have been discarded!
        ...
        # Step 4: Delete box B (index 1 in boxes list)
        ...
        # Step 5: Delete box C (index 2 in boxes list)
...
def test_2_process_pages_vs_repaint_mask() -> None:
    """Scenario 11: process_pages x repaint_mask
...
def test_3_process_pages_vs_add_manual_box() -> None:
    """Scenario 12: process_pages x add_manual_box
...
def test_4_process_pages_vs_update_box() -> None:
    """Scenario 13: process_pages x update_box
...
def test_5_process_pages_vs_reset_manual_mask() -> None:
    """Scenario 14: process_pages x reset_manual_mask
...
def test_6_process_pages_vs_mark_skipped() -> None:
    """Scenario 15: process_pages x mark_skipped
...
def test_7_process_pages_vs_save_excluded_regions() -> None:
    """Scenario 16: process_pages x save_excluded_regions
