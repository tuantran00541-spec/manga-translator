#!/usr/bin/env python3
"""Profile the production YoloDetector without changing its behavior.

Profiles one or more images and reports slice count plus per-slice timing for:
- preprocess
- ONNX Runtime session.run
- postprocess (including NMS/mask work)
- mask decode (subset of postprocess)
- final cross-slice NMS

TTA is intentionally off unless explicitly requested. This is diagnostic only;
it does not modify production detector code or model outputs.
"""
...
class ProfiledYoloDetectorMixin:
    ...
    def _nms_boxes(self, boxes):
        # This method is used for the cross-slice merge when TTA is off.
        # It may also be used by TTA; the output count is still recorded.
        t0 = time.perf_counter()
...
def build_profiled_detector(repo: Path, model: Path, conf: float, threads: int, tta: bool):
    ...
    # Dynamic subclass so we inherit the exact production implementation.
