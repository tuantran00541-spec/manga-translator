...
def run_benchmark():
    ...
    # Collect test images from RAW_DIR or create a test dummy image
    image_paths = []
    ...
    if not image_paths:
        ...
        # Draw some speech bubbles and text-like boxes
        ...
        test_images = []
        for p in image_paths[:10]:  # Limit to 10 images max
            img = cv2.imread(p)
            if img is not None:
    ...
    for name, img in test_images:
        ...
        # Match boxes to find new boxes and confidence boosts
        matched_tta = set()
        new_in_page = 0
