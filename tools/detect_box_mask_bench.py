#!/usr/bin/env python3
"""Diagnostic benchmark for the production detect -> box -> mask pipeline.

This tool NEVER modifies production detector code. It imports the existing
YoloDetector / CombinedTextDetector implementation and records:
  - model/provider identity
  - preprocessing scale + padding + slice geometry
  - raw bubble/text detector boxes
  - final CombinedTextDetector boxes
  - mask coverage / box coverage / edge contact / geometry diagnostics
  - optional baseline-vs-TTA comparison
  - visual overlays for manual inspection

Usage (from repo root):
  python tools/detect_box_mask_bench.py \
      --images path/to/a.jpg path/to/b.jpg \
      --bubble-model models/bubble_yolo.onnx \
      --text-model models/text_segmenter.onnx \
      --out bench/detect_box_mask

Optional:
  --tta                 run a second pass with production TTA enabled
  --conf 0.40          bubble confidence threshold
  --text-conf 0.20     text confidence threshold
  --save-masks         save final binary masks as PNGs
  --max-images N       cap number of input images

The script is intentionally diagnostic: without ground-truth annotations it
must NOT report precision/recall/IoU as correctness scores.
"""
...
# Make repo-root imports work when invoked as `python tools/...py`.
ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from app.config import BUBBLE_IOU_THRESHOLD, ENABLE_TTA, BUBBLE_CONF_THRESHOLD, TEXT_CONF_THRESHOLD  # noqa: E402
from app.detector.bubble_detector import BubbleBox, YoloDetector  # noqa: E402
from app.detector.combined_detector import CombinedTextDetector  # noqa: E402
from app.ort_utils import make_session  # noqa: E402
