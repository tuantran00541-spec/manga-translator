#!/usr/bin/env python3
"""Detect -> box -> mask correctness benchmark v3 (speech-bubble scope).

Trust-boundary instrumentation only: production source is NOT modified.

The benchmark executes production detector/inpainter code while instrumenting:
- raw bubble/text detector calls
- exact CombinedTextDetector post-processing with detector outputs cached
- association counts / ambiguous assignments
- free-text clustering and line splitting
- tall-box refinement and mask/box synchronization
- final NMS
- production build_mask + adaptive dilation (pre/post dilation)
- production Inpainter clustering/crop geometry

The key design choice is that the combined stage reuses the already measured raw
bubble/text outputs. This avoids the old benchmark's duplicate detector inference
inside CombinedTextDetector.detect(), while still executing the production
post-processing implementation byte-for-byte.
"""
...
# Allow direct execution from <repo>/tools/ while resolving the top-level
# ``app`` package from the repository root.
...
def target_scope_probe(bubbles: list[BubbleBox], texts: list[BubbleBox], detector: CombinedTextDetector) -> dict[str, Any]:
    """Diagnostic only: classify raw text as bubble-associated vs standalone.

    This is NOT ground truth. A text box outside every detected bubble is
    reported as standalone/out_of_scope_candidate so SFX/captions do not get
    counted as speech-text misses.
    """
    per_bubble = []
    for bi, b in enumerate(bubbles):
        inside = [ti for ti, t in enumerate(texts) if detector._is_inside(t, b)]
        per_bubble.append({
            "bubble_index": bi,
            "text_indices": inside,
            "has_text_candidate": bool(inside),
        })
    outside = []
    for ti, t in enumerate(texts):
        if not any(detector._is_inside(t, b) for b in bubbles):
            outside.append(ti)
    return {
        "candidate_bubbles": len(bubbles),
        "candidate_bubbles_with_text": sum(x["has_text_candidate"] for x in per_bubble),
        "candidate_bubbles_without_text": sum(not x["has_text_candidate"] for x in per_bubble),
        "standalone_text_candidates": outside,
        "per_bubble": per_bubble,
        "warning": "Not ground truth; standalone candidates are excluded from speech-bubble quality scoring.",
    }
...
def run_combined_cached(combined: CombinedTextDetector, image: np.ndarray, bubble_boxes: list[BubbleBox], text_boxes: list[BubbleBox], trace: Trace) -> tuple[list[BubbleBox], float]:
    """Execute the exact production CombinedTextDetector.detect post-processing.

    Detector inference is replaced only for this benchmark invocation by cached
    outputs captured from the same detector objects immediately beforehand.
    """
...
def instrument_mask_builder(trace: Trace) -> dict[str, Any]:
    ...
    def build_wrapped(image_shape, boxes, crop_img=None):
        t0 = time.perf_counter()
        # The original build_mask calls mask_builder.adaptive_dilate_mask, which
        # is now wrapped, so this captures both pre/post dilation indirectly.
...
def run_one(image_path: Path, combined: CombinedTextDetector, inpainter: Inpainter, out_dir: Path, save_masks: bool) -> dict[str, Any]:
    ...
    # Raw detector calls are the only model inference in this benchmark pass.
    t0 = time.perf_counter()
    ...
    # Exact production mask construction. We do not call this as a proxy metric:
    # execute Inpainter.inpaint() with a harmless session call bypassed below.
    originals = instrument_mask_builder(trace)
    inpainter_originals = instrument_inpainter_methods(inpainter, trace)
    try:
        # Replace only the model session call, because this stage benchmark is
        # about detector->mask construction. _smart_paint_region is intentionally
        # not executed here; production mask construction itself is.
    ...
    return {
        ...
        "association": assoc,
        "target_scope": target_scope_probe(bubble_boxes, text_boxes, combined),
...
def main() -> int:
    ...
    ap.add_argument("--images", nargs="+", required=True, help="Images or glob patterns; Python expands wildcards itself.")
    ...
    ap.add_argument("--out", default="bench/detect_box_mask_v3")
    ap.add_argument("--write-annotation-template", action="store_true", help="Write a human-review GT template from raw candidates.")
    ...
    images = sorted({Path(x).resolve() for x in expanded})
    ...
    payload = {
        ...
        "benchmark": "detect_box_mask_v3",
        "scope": "speech_bubble_text_only",
        ...
        "summary": {
            ...
            "candidate_bubbles_with_text_total": sum(x["target_scope"]["candidate_bubbles_with_text"] for x in results),
            "candidate_bubbles_without_text_total": sum(x["target_scope"]["candidate_bubbles_without_text"] for x in results),
            "standalone_text_candidate_total": sum(len(x["target_scope"]["standalone_text_candidates"]) for x in results),
    ...
    if args.write_annotation_template:
        template = {
            ...
            "scope": "speech_bubble_text_only",
            "instructions": [
                "Mark each detected bubble candidate as target speech bubble / non-target / uncertain.",
                "Do not score SFX, cover titles, credits, captions, UI text, or decorative typography as target speech text.",
                "Mark each raw text candidate as target speech text / non-target / uncertain.",
                "For each target speech bubble, add one or more GT text boxes in source-image coordinates.",
            ],
            "images": [
                {
                    "image": r["image"],
        ...
        tpath = out_dir / "ground_truth_template.json"
